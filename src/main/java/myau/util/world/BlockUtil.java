    ...
    Vec3 vec3 =
        new Vec3(
            (double) blockPos.getX()
                + Math.min(
                    Math.max(RandomUtil.nextDouble(0.0, 1.0), block.getBlockBoundsMinX()),
                    block.getBlockBoundsMaxX()),
            (double) blockPos.getY()
                + Math.min(
                    Math.max(RandomUtil.nextDouble(0.0, 1.0), block.getBlockBoundsMinY()),
                    block.getBlockBoundsMaxY()),
            (double) blockPos.getZ()
                + Math.min(
                    Math.max(RandomUtil.nextDouble(0.0, 1.0), block.getBlockBoundsMinZ()),
                    block.getBlockBoundsMaxZ()));
    switch (enumFacing) {
      default:
        {
          return new Vec3(
              vec3.xCoord, (double) blockPos.getY() + block.getBlockBoundsMinY(), vec3.zCoord);
        }
      case UP:
        {
          return new Vec3(
              vec3.xCoord, (double) blockPos.getY() + block.getBlockBoundsMaxY(), vec3.zCoord);
        }
      case NORTH:
        {
          return new Vec3(
              vec3.xCoord, vec3.yCoord, (double) blockPos.getZ() + block.getBlockBoundsMinZ());
        }
      case EAST:
        {
          return new Vec3(
              (double) blockPos.getX() + block.getBlockBoundsMaxX(), vec3.yCoord, vec3.zCoord);
        }
      case SOUTH:
        {
          return new Vec3(
              vec3.xCoord, vec3.yCoord, (double) blockPos.getZ() + block.getBlockBoundsMaxZ());
        }
      case WEST:
    }
    return new Vec3(
        (double) blockPos.getX() + block.getBlockBoundsMinX(), vec3.yCoord, vec3.zCoord);
  }
}
