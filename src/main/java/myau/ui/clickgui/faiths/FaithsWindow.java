...
public class FaithsWindow {
  ...
  protected void renderWindow(int mouseX, int mouseY) {
    ...
    List<Module> modules = Myau.moduleManager.getModulesByCategory().get(category);
    float height = 15F;
    ...
    scrollY = myau.util.math.MathUtil.lerp(scrollY, targetScrollY, 0.2f);
    ...
    if (modules != null && expand) {
      for (Module module : modules) {
        ...
        Color c1 =
            myau.util.render.Themes.getCurrentTheme()
                .getAccentColor(new myau.util.vector.Vector2d(x, y + moduleY));
        Color c2 =
            myau.util.render.Themes.getCurrentTheme()
                .getAccentColor(
                    new myau.util.vector.Vector2d(x + PANEL_WIDTH, y + moduleY + MODULE_HEIGHT));
