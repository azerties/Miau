package myau.ui.clickgui.faiths;
...
public class FaithsCharacterRenderer {
  ...
  private static void scanAssetCharacters(List<String> list) {
    try {
      java.net.URL url = myau.Myau.class.getResource("/assets/keystrokesmod/textures/gui/faiths/");
      if (url != null) {
  ...
  public static List<String> getAvailableCharacters() {
    ...
    File dir = new File("./config/Myau/characters/");
    if (!dir.exists()) {
      dir.mkdirs();
    } else {
      File[] files =
  ...
  private static void loadTexture(String charName) {
    ...
    if (src == null) {
      ...
      for (String ext : extensions) {
        ...
        try (InputStream stream = myau.Myau.class.getResourceAsStream(assetPath)) {
          if (stream != null) {
    ...
    if (src == null && !loadedCharName.equals("character")) {
      try (InputStream stream =
          myau.Myau.class.getResourceAsStream(
  ...
  public static void renderCharacter(float delta) {
    ...
    try {
      ClickGUI clickGuiMod = (ClickGUI) myau.Myau.moduleManager.getModule(ClickGUI.class);
