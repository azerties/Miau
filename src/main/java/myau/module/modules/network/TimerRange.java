...
public class TimerRange extends Module {
  ...
  public final ModeProperty timerBoostMode =
      new ModeProperty("mode", 2, new String[] {"NORMAL", "SMART", "MODERN"});
  ...
  public final FloatProperty boostDelay = new FloatProperty("boost-delay", 0.5F, 0.55F, 0.1F, 1.0F);

  public final FloatProperty timerChargedValue =
      new FloatProperty("timer-charged", 0.45F, 0.05F, 5.0F);
  public final FloatProperty chargedDelay =
      new FloatProperty("charged-delay", 0.75F, 0.9F, 0.1F, 1.0F);

  public final FloatProperty rangeValue =
      new FloatProperty("range", 3.5F, 1.0F, 5.0F, () -> timerBoostMode.getValue() == 0);
  public final IntProperty cooldownTickValue =
      new IntProperty("cooldown-tick", 10, 1, 50, () -> timerBoostMode.getValue() == 0);

  public final FloatProperty rangeBound =
      new FloatProperty("rangeBound", 2.5F, 3.0F, 2.0F, 8.0F, () -> timerBoostMode.getValue() != 0);
  public final FloatProperty scanRange =
      new FloatProperty("scan-range", 8.0F, 2.0F, 12.0F, () -> timerBoostMode.getValue() != 0);

  public final FloatProperty tickDelay =
      new FloatProperty(
          "tick-delay", 30.0F, 60.0F, 1.0F, 200.0F, () -> timerBoostMode.getValue() != 0);

  public final BooleanProperty blink = new BooleanProperty("blink", false);
  public final IntProperty predictClientMovement =
      new IntProperty("predict-client-movement", 2, 0, 5);
  public final FloatProperty predictEnemyPosition =
      new FloatProperty("predict-enemy-position", 1.5F, -1.0F, 2.0F);
  public final FloatProperty maxAngleDifference =
      new FloatProperty(
          "max-angle-difference", 5.0F, 5.0F, 90.0F, () -> timerBoostMode.getValue() == 2);

  public final ModeProperty markMode =
      new ModeProperty(
          "mark", 0, new String[] {"OFF", "BOX", "PLATFORM"}, () -> timerBoostMode.getValue() == 2);
  public final BooleanProperty outline =
      new BooleanProperty(
          "outline", false, () -> timerBoostMode.getValue() == 2 && markMode.getValue() == 1);
  ...
  public final BooleanProperty chatDebug =
      new BooleanProperty(
          "chat-debug", true, () -> resetOnLagBack.getValue() || resetOnKnockback.getValue());
  ...
  public void onAttack(AttackEvent event) {
    ...
    int randomTickDelay =
        RandomUtil.nextInt(tickDelay.getValue().intValue(), tickDelay.getSecondValue().intValue());

    boolean shouldReturn =
        BackTrack.runWithNearestTrackedDistance(targetEntity, () -> !updateDistance(targetEntity));

    if (shouldReturn
        || (mc.thePlayer.isInWater() && !onLiquid.getValue())
        || (mc.thePlayer.isInLava() && !onLiquid.getValue())) {
      return;
    }
  ...
  public void onTick(TickEvent event) {
    if (event.getType() == EventType.PRE) {
      float timerBoost = RandomUtil.nextFloat(boostDelay.getValue(), boostDelay.getSecondValue());
      float charged = RandomUtil.nextFloat(chargedDelay.getValue(), chargedDelay.getSecondValue());

      if (mc.thePlayer != null && mc.theWorld != null) {
        randomRange = RandomUtil.nextFloat(rangeBound.getValue(), rangeBound.getSecondValue());
      }
      ...
      if (playerTicks <= 0 || confirmStop) {
        ...
        float speedAdjustment =
            playerSpeed >= 0 ? playerSpeed : 1f + ticksValue.getValue() - playerTicks;
      ...
      int randomTickDelay =
          RandomUtil.nextInt(
              tickDelay.getValue().intValue(), tickDelay.getSecondValue().intValue());

      boolean shouldReturn2 =
          BackTrack.runWithNearestTrackedDistance(
              nearbyEntity, () -> !updateDistance(nearbyEntity));
      ...
      if (isPlayerMoving() && !confirmStop) {
        if (isLookingOnEntities(nearbyEntity, maxAngleDifference.getValue())) {
          ...
          if (confirmTick
              && entityDistance >= randomRange
              && entityDistance <= rangeBound.getSecondValue()) {
      ...
      if (blinked && event.getType() == EventType.RECEIVE) {
        if (packet instanceof S08PacketPlayerPosLook
          ...
          if (packet instanceof S27PacketExplosion) {
            ...
            if (expl.func_149149_c() != 0f
                || expl.func_149144_d() != 0f
                || expl.func_149147_e() != 0f) {
              unblink();
              return;
            }
    ...
    if (event.getType() == EventType.RECEIVE) {
      if (resetOnLagBack.getValue() && packet instanceof S08PacketPlayerPosLook) {
        ...
        if (shouldReset) {
          if (chatDebug.getValue())
            ChatUtil.sendFormatted("§c[TimerRange] §fLagback Received | AnimationTimer Reset");
          shouldReset = false;
        }
      }
      ...
      if (resetOnKnockback.getValue() && packet instanceof S12PacketEntityVelocity) {
        if (((S12PacketEntityVelocity) packet).getEntityID() == mc.thePlayer.getEntityId()) {
          ...
          if (shouldReset) {
            if (chatDebug.getValue())
              ChatUtil.sendFormatted("§c[TimerRange] §fKnockback Received | AnimationTimer Reset");
            shouldReset = false;
          }
        }
      }
  ...
  public void onRender3D(Render3DEvent event) {
    if (mc.thePlayer == null
        ...
        || timerBoostMode.getValue() != 2
        || markMode.getValue() == 0) return;

    EntityLivingBase nearbyEntity = getNearestEntityInRange();
    if (nearbyEntity != null
        && mc.thePlayer.getDistanceToEntity(nearbyEntity) <= scanRange.getValue()) {
      Color color =
          isLookingOnEntities(nearbyEntity, maxAngleDifference.getValue())
              ? new Color(37, 126, 255, 70)
              : new Color(210, 60, 60, 70);

      float partialTicks = event.getPartialTicks();
      double x =
          nearbyEntity.lastTickPosX
              + (nearbyEntity.posX - nearbyEntity.lastTickPosX) * partialTicks
              - mc.getRenderManager().viewerPosX;
      double y =
          nearbyEntity.lastTickPosY
              + (nearbyEntity.posY - nearbyEntity.lastTickPosY) * partialTicks
              - mc.getRenderManager().viewerPosY;
      double z =
          nearbyEntity.lastTickPosZ
              + (nearbyEntity.posZ - nearbyEntity.lastTickPosZ) * partialTicks
      ...
      if (markMode.getValue() == 1) {
        ...
        RenderUtil.drawBoundingBox(
            aabb, color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha(), 1.8F);
      } else if (markMode.getValue() == 2) {
        AxisAlignedBB aabb =
            new AxisAlignedBB(x - width, y, z - width, x + width, y + 0.1D, z + width);
        RenderUtil.drawBoundingBox(
            aabb, color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha(), 1.8F);
      }
    }
  ...
  private EntityLivingBase getNearestEntityInRange() {
    ...
    for (Entity e : mc.theWorld.loadedEntityList) {
      if (e instanceof EntityPlayer && e != mc.thePlayer && !e.isDead) {
        ...
        double maxCheck =
            timerBoostMode.getValue() == 0
                ? rangeValue.getValue()
                : (scanRange.getValue() + randomRange);
  ...
  private void setTimerSpeed(float speed) {
    try {
      ...
      try {
        speedField = timerObj.getClass().getDeclaredField("timerSpeed");
      } catch (NoSuchFieldException e) {
        speedField = timerObj.getClass().getDeclaredField("field_74278_d");
      }
