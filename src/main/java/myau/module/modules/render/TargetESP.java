...
public class TargetESP extends Module {
  ...
  public final IntProperty red =
      new IntProperty("red", 255, 0, 255, () -> !this.hudColor.getValue());
  public final IntProperty green =
      new IntProperty("green", 80, 0, 255, () -> !this.hudColor.getValue());
  public final IntProperty blue =
      new IntProperty("blue", 140, 0, 255, () -> !this.hudColor.getValue());
  ...
  public void onRender3D(Render3DEvent event) {
    ...
    double x =
        lerp(target.lastTickPosX, target.posX, event.getPartialTicks())
            - ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosX();
    double y =
        lerp(target.lastTickPosY, target.posY, event.getPartialTicks())
            - ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosY();
    double z =
        lerp(target.lastTickPosZ, target.posZ, event.getPartialTicks())
  ...
  private void renderMiauHalo(EntityLivingBase target, double x, double y, double z, Color color) {
    ...
    float pulseValue =
        this.pulse.getValue()
            ...
            : 0.5F;
    double r =
        (target.width * 0.75D + this.radius.getValue())
            * (0.92D + pulseValue * 0.08D + hurtPulse * 0.10D);
  ...
  private void renderGroundCircle(
      EntityLivingBase target, double x, double y, double z, Color color) {
    float hurtPulse = target.hurtTime > 0 ? target.hurtTime / 10.0F : 0.0F;
    float red = color.getRed() / 255.0F;
    float green = color.getGreen() / 255.0F;
    float blue = color.getBlue() / 255.0F;
  ...
  private void renderDoubleSpiral(
      EntityLivingBase target, double x, double y, double z, Color color) {
    float red = color.getRed() / 255.0F;
    float green = color.getGreen() / 255.0F;
    float blue = color.getBlue() / 255.0F;
    ...
    for (int spiral = 0; spiral < 2; spiral++) {
      ...
      for (int i = 0; i < points; i++) {
        ...
        double spin =
            (now / (spiral == 0 ? 260.0D : -260.0D))
                + progress * Math.PI * 2.15D
                + spiral * Math.PI;
