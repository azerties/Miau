...
public class Xray extends Module {
  ...
  private void renderOreHighlight(BlockPos blockPos, int blockId, Vec3 viewVector) {
    if (mc.thePlayer.getDistance(blockPos.getX(), blockPos.getY(), blockPos.getZ())
        <= this.range.getValue().doubleValue()) {
      Color color = this.getOreColor(blockId);
      RenderUtil.drawBlockBoundingBox(
          blockPos, 1.0, color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha(), 1.5F);
  ...
  public void onRender3D(Render3DEvent event) {
    if (this.isEnabled()) {
      ...
      vec3 =
          new Vec3(
              vec3.xCoord,
              vec3.yCoord + (double) mc.getRenderViewEntity().getEyeHeight(),
              vec3.zCoord);
  ...
  public void onPacket(PacketEvent event) {
    if (event.getType() == EventType.RECEIVE) {
      if (event.getPacket() instanceof S22PacketMultiBlockChange) {
        for (BlockUpdateData blockUpdateData :
