...
public class Radar extends Module {
  ...
  public final IntProperty offsetX =
      new IntProperty("offset-x", 60, 0, 1000, () -> position.getValue() != 4);
  public final IntProperty offsetY =
      new IntProperty("offset-y", 60, 0, 1000, () -> position.getValue() != 4);
  ...
  public final ColorProperty outlineColor =
      new ColorProperty("outline-color", Color.DARK_GRAY.getRGB());
  public final ColorProperty crossColor =
      new ColorProperty("cross-color", Color.LIGHT_GRAY.getRGB());
  ...
  private boolean shouldRender(EntityPlayer entityPlayer) {
    if (entityPlayer.deathTime > 0) {
      ...
      if (TeamUtil.isBot(entityPlayer)) {
        ...
        return TeamUtil.isTarget(entityPlayer)
            ? this.showEnemies.getValue()
            : this.showPlayers.getValue();
      }
    } else {
      return false;
    }
  ...
  public void onRender(Render2DEvent event) {
    ...
    if (position.getValue() == 4) {
      ...
      centerX =
          (position.getValue() & 0x1) == 0x1
              ? Math.max(sr.getScaledWidth() - offsetX.getValue(), 0)
              : Math.min(offsetX.getValue(), sr.getScaledWidth());
      centerY =
          (position.getValue() & 0x2) == 0x2
              ? Math.max(sr.getScaledHeight() - offsetY.getValue(), 0)
              : Math.min(offsetY.getValue(), sr.getScaledHeight());
    }
    ...
    this.drawRadarCircle(
        ...
        0,
        yaw,
        radarRadius.getValue(),
        64,
        new Color(fill.getRed(), fill.getGreen(), fill.getBlue(), 100).getRGB(),
    ...
    for (EntityPlayer player :
        TeamUtil.getLoadedEntitiesSorted().stream()
            .filter(
                entity ->
                    entity instanceof EntityPlayer && this.shouldRender((EntityPlayer) entity))
  ...
  public void drawRadarCircle(
    ...
    if ((fillColor >>> 24) != 0) {
      ...
      for (int i = 0; i <= segments; i++) {
        ...
        GL11.glVertex2d(x + Math.cos(angle1) * radius, y + Math.sin(angle1) * radius);
      }
      GL11.glEnd();
    }
