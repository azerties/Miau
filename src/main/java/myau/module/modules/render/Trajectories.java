    ...
    if (this.isEnabled()
      ...
      double mx =
          (double)
              ...
              * -1.0;
      double my =
          (double) MathHelper.sin(pitch / 180.0F * (float) Math.PI) * (isBow ? 1.0 : 0.4) * -1.0;
      double mz =
          (double)
      ...
      while (!hasHitBlock && y > 0.0) {
        ...
        for (Entity entity : possibleEntities) {
          if (entity.canBeCollidedWith() && entity != mc.thePlayer) {
            AxisAlignedBB entityBox =
                entity.getEntityBoundingBox().expand(hitboxExpand, hitboxExpand, hitboxExpand);
        ...
        y += my;
        z += mz;
        if (mc.theWorld.getBlockState(new BlockPos(x, y, z)).getBlock().getMaterial()
            == Material.water) {
          mx *= 0.6;
          my *= 0.6;
          mz *= 0.6;
      ...
      if (trajectoryPoints.size() > 1) {
        ...
        trajectoryPoints.forEach(
            vec3 -> worldRenderer.pos(vec3.xCoord, vec3.yCoord, vec3.zCoord).endVertex());
