package myau.clientanticheat.player.scaffold.ml;

import java.util.Random;

/**
 * Pooling strategies for converting variable-length RNN outputs into a fixed-size vector. Ported
 * from MX Project's kireiko.dev.millennium.ml.logic.rnn.pooling
 */

// ── Pooling interface ──
interface PoolingStrategy {
  int outputSize();

  double[] forward(double[][] hTime, double[] mask, PoolingCache cache);

  double[][] backward(
      double[][] hTime, double[] mask, double[] dPooled, PoolingCache cache, PoolingGrad gAcc);
}

// ── Cache / Grad holders ──
final class PoolingCache {
  double[] attnWeights;
  double[] attnScores;
}

final class PoolingGrad {
  double[] dAttentionW;
  double dAttentionB;
}

// ── Last Hidden Pooling ──
final class LastHiddenPooling implements PoolingStrategy {
  private final int outSize;

  LastHiddenPooling(int outSize) {
    this.outSize = outSize;
  }

  @Override
  public int outputSize() {
    return outSize;
  }

  @Override
  public double[] forward(double[][] hTime, double[] mask, PoolingCache cache) {
    int last = -1;
    for (int t = hTime.length - 1; t >= 0; t--) {
      if (mask[t] > 0.5) {
        last = t;
        break;
      }
    }
    if (last < 0) last = hTime.length - 1;
    return hTime[last].clone();
  }

  @Override
  public double[][] backward(
      double[][] hTime, double[] mask, double[] dPooled, PoolingCache cache, PoolingGrad gAcc) {
    double[][] dH = new double[hTime.length][outSize];
    int last = -1;
    for (int t = hTime.length - 1; t >= 0; t--) {
      if (mask[t] > 0.5) {
        last = t;
        break;
      }
    }
    if (last < 0) last = hTime.length - 1;
    System.arraycopy(dPooled, 0, dH[last], 0, outSize);
    return dH;
  }
}

// ── Mean Pooling ──
final class MeanPooling implements PoolingStrategy {
  private final int outSize;

  MeanPooling(int outSize) {
    this.outSize = outSize;
  }

  @Override
  public int outputSize() {
    return outSize;
  }

  @Override
  public double[] forward(double[][] hTime, double[] mask, PoolingCache cache) {
    int T = hTime.length;
    double[] pooled = new double[outSize];
    int count = 0;
    for (int t = 0; t < T; t++) {
      if (mask[t] <= 0.5) continue;
      count++;
      for (int i = 0; i < outSize; i++) pooled[i] += hTime[t][i];
    }
    if (count > 0) {
      double inv = 1.0 / count;
      for (int i = 0; i < outSize; i++) pooled[i] *= inv;
    }
    return pooled;
  }

  @Override
  public double[][] backward(
      double[][] hTime, double[] mask, double[] dPooled, PoolingCache cache, PoolingGrad gAcc) {
    int T = hTime.length;
    double[][] dH = new double[T][outSize];
    int count = 0;
    for (int t = 0; t < T; t++) if (mask[t] > 0.5) count++;
    if (count > 0) {
      double inv = 1.0 / count;
      for (int t = 0; t < T; t++) {
        if (mask[t] <= 0.5) continue;
        for (int i = 0; i < outSize; i++) dH[t][i] = dPooled[i] * inv;
      }
    }
    return dH;
  }
}

// ── Max Pooling ──
final class MaxPooling implements PoolingStrategy {
  private final int outSize;

  MaxPooling(int outSize) {
    this.outSize = outSize;
  }

  @Override
  public int outputSize() {
    return outSize;
  }

  @Override
  public double[] forward(double[][] hTime, double[] mask, PoolingCache cache) {
    double[] pooled = new double[outSize];
    for (int i = 0; i < outSize; i++) pooled[i] = Double.NEGATIVE_INFINITY;

    for (int t = 0; t < hTime.length; t++) {
      if (mask[t] <= 0.5) continue;
      for (int i = 0; i < outSize; i++) {
        if (hTime[t][i] > pooled[i]) pooled[i] = hTime[t][i];
      }
    }
    for (int i = 0; i < outSize; i++) {
      if (pooled[i] == Double.NEGATIVE_INFINITY) pooled[i] = 0.0;
    }
    return pooled;
  }

  @Override
  public double[][] backward(
      double[][] hTime, double[] mask, double[] dPooled, PoolingCache cache, PoolingGrad gAcc) {
    double[][] dH = new double[hTime.length][outSize];
    for (int i = 0; i < outSize; i++) {
      int maxT = -1;
      double maxV = Double.NEGATIVE_INFINITY;
      for (int t = 0; t < hTime.length; t++) {
        if (mask[t] <= 0.5) continue;
        if (hTime[t][i] > maxV) {
          maxV = hTime[t][i];
          maxT = t;
        }
      }
      if (maxT >= 0) dH[maxT][i] = dPooled[i];
    }
    return dH;
  }
}

// ── Attention Pooling ──
final class AttentionPooling implements PoolingStrategy {
  private final int outSize;
  final double[] W;
  double b;

  AttentionPooling(int outSize, Random rng) {
    this.outSize = outSize;
    this.W = MathOps.xavierUniform(rng, outSize, outSize, 1);
    this.b = 0.0;
  }

  @Override
  public int outputSize() {
    return outSize;
  }

  @Override
  public double[] forward(double[][] hTime, double[] mask, PoolingCache cache) {
    int T = hTime.length;
    double[] scores = new double[T];
    double maxScore = Double.NEGATIVE_INFINITY;

    for (int t = 0; t < T; t++) {
      if (mask[t] <= 0.5) {
        scores[t] = Double.NEGATIVE_INFINITY;
        continue;
      }
      double s = b;
      for (int i = 0; i < outSize; i++) s += hTime[t][i] * W[i];
      scores[t] = s;
      if (s > maxScore) maxScore = s;
    }

    double sumExp = 0.0;
    double[] a = new double[T];
    if (maxScore > Double.NEGATIVE_INFINITY) {
      for (int t = 0; t < T; t++) {
        if (mask[t] <= 0.5) continue;
        double e = Math.exp(scores[t] - maxScore);
        a[t] = e;
        sumExp += e;
      }
    }
    double inv = 1.0 / (sumExp + 1e-10);
    for (int t = 0; t < T; t++) a[t] *= inv;

    double[] pooled = new double[outSize];
    for (int t = 0; t < T; t++) {
      double at = a[t];
      if (at == 0.0) continue;
      for (int i = 0; i < outSize; i++) pooled[i] += at * hTime[t][i];
    }

    if (cache != null) {
      cache.attnWeights = a;
      cache.attnScores = scores;
    }
    return pooled;
  }

  @Override
  public double[][] backward(
      double[][] hTime, double[] mask, double[] dPooled, PoolingCache cache, PoolingGrad gAcc) {
    int T = hTime.length;
    double[] a = cache.attnWeights;
    double[][] dH = new double[T][outSize];

    double[] dAlpha = new double[T];
    for (int t = 0; t < T; t++) {
      if (mask[t] <= 0.5) continue;
      double s = 0.0;
      for (int i = 0; i < outSize; i++) s += dPooled[i] * hTime[t][i];
      dAlpha[t] = s;
    }

    double sum = 0.0;
    for (int t = 0; t < T; t++) sum += a[t] * dAlpha[t];

    double[] dScore = new double[T];
    for (int t = 0; t < T; t++) {
      if (mask[t] <= 0.5) continue;
      dScore[t] = a[t] * (dAlpha[t] - sum);
    }

    for (int t = 0; t < T; t++) {
      if (mask[t] <= 0.5) continue;
      double at = a[t];
      for (int i = 0; i < outSize; i++) dH[t][i] += at * dPooled[i];

      double ds = dScore[t];
      for (int i = 0; i < outSize; i++) {
        gAcc.dAttentionW[i] += ds * hTime[t][i];
        dH[t][i] += ds * W[i];
      }
      gAcc.dAttentionB += ds;
    }
    return dH;
  }
}
