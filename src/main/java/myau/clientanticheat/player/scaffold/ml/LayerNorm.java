...
/**
 * Layer Normalization for LSTM hidden states. Ported from MX Project's
 * kireiko.dev.millennium.ml.logic.rnn.layers.LayerNorm
 */
final class LayerNorm {
  ...
  static double[] forward(double[] x, double[] gamma, double[] beta, Cache c) {
    int n = x.length;
    double mean = 0.0;
  ...
  static double[] backward(double[] dY, double[] gamma, Cache c, double[] dGamma, double[] dBeta) {
    int n = dY.length;

    double[] dXhat = new double[n];
    for (int i = 0; i < n; i++) {
      dGamma[i] += dY[i] * c.xHat[i];
      dBeta[i] += dY[i];
