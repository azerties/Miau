...
public class AntiBot extends Module {
  ...
  public final ModeProperty tabMode =
      new ModeProperty("TabMode", 1, new String[] {"Equals", "Contains"}, tab::getValue);
  ...
  public final IntProperty livingTimeTicks =
      new IntProperty("LivingTimeTicks", 40, 1, 200, livingTime::getValue);
  ...
  public final FloatProperty healthBound =
      new FloatProperty("HealthBound", 0.0F, 20.0F, 0.0F, 100.0F, health::getValue);
  ...
  public final ModeProperty duplicateCompareMode =
      new ModeProperty(
          "DuplicateCompareMode",
          0,
          new String[] {"OnTime", "WhenSpawn"},
          () -> duplicateInTab.getValue() || duplicateInWorld.getValue());
  public final BooleanProperty experimentalNPCDetection =
      new BooleanProperty("ExperimentalNPCDetection", false);
  ...
  public final IntProperty removeInterval =
      new IntProperty("Remove-Interval", 20, 1, 100, removeFromWorld::getValue);
  ...
  public void onTick(TickEvent event) {
    ...
    if (!removeFromWorld.getValue()
        ...
        || mc.thePlayer.ticksExisted % removeInterval.getValue() != 0) return;
  ...
  private void handleMatrixBot() {
    ...
    if (matrixCollectSample) {
      ...
      for (EntityPlayer bot : bots) {
        ...
        if (debug.getValue()) {
          ChatUtil.display(
              "%sAntiBot/MatrixBot: &fRemoved &r%s&f. Speed: &b%.2f&f.",
              bot.getName(), getHorizontalSpeed(bot));
        }
      }
    }
  ...
  private void handleEntityPacket(S14PacketEntity packet) {
    ...
    if (packet.func_149060_h()) {
      if (player.prevPosY != player.posY)
        invalidGroundVL.put(id, invalidGroundVL.getOrDefault(id, 0) + 1);
    } else {
  ...
  private void handleSpawnPlayerPacket(S0CPacketSpawnPlayer packet) {
    ...
    if (killAura != null
        ...
        && !hasRemovedEntities.contains(packet.getEntityID())) {
  ...
  public boolean isBotPlayer(EntityLivingBase entity) {
    ...
    if (illegalName.getValue()
        && (player.getName().contains(" ")
            || player.getDisplayName().getUnformattedText().contains(" "))) return true;
    if (color.getValue()
        && !player.getDisplayName().getFormattedText().replace("§r", "").contains("§")) return true;
    ...
    if (health.getValue()
        && (player.getHealth() > healthBound.getSecondValue()
            || player.getHealth() < healthBound.getValue())) return true;
    if (entityID.getValue() && (id >= 1000000000 || id <= -1)) return true;
    if (derp.getValue() && (player.rotationPitch > 90.0F || player.rotationPitch < -90.0F))
      return true;
    ...
    if (duplicateCompareMode.getValue() == 1 && duplicate.contains(player.getGameProfile().getId()))
      return true;
    if (duplicateInWorld.getValue()
        && duplicateCompareMode.getValue() == 0
        && hasDuplicateInWorld(player)) return true;
    if (duplicateInTab.getValue()
        && duplicateCompareMode.getValue() == 0
        && hasDuplicateInTab(player)) return true;
  ...
  private boolean isInTab(EntityPlayer player) {
    ...
    for (NetworkPlayerInfo info : mc.getNetHandler().getPlayerInfoMap()) {
      ...
      if (equals ? targetName.equalsIgnoreCase(networkName) : targetName.contains(networkName))
        return true;
    }
    return false;
  }
            ...
            .filter(
                entity ->
                    entity instanceof EntityPlayer
                        && name.equals(((EntityPlayer) entity).getName()))
            .count()
        > 1;
  }
  ...
  private boolean hasDuplicateInTab(EntityPlayer player) {
    ...
    return mc.getNetHandler().getPlayerInfoMap().stream()
            .filter(
                info ->
                    info.getGameProfile() != null && name.equals(info.getGameProfile().getName()))
            .count()
        > 1;
  }
