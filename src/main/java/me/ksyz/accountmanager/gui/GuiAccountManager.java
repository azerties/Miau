...
public class GuiAccountManager extends GuiScreen {
  ...
  public void drawScreen(int mouseX, int mouseY, float renderPartialTicks) {
    ...
    drawCenteredString(
        ...
        TextFormatting.translate(
            String.format("&rAccount Manager &8(&7%s&8)&r", AccountManager.accounts.size())),
        width / 2,
        20,
        -1);
    ...
    String text =
        TextFormatting.translate(
            String.format("&7Username: &3%s&r", SessionManager.get().getUsername()));
    mc.currentScreen.drawString(mc.fontRendererObj, text, 3, 3, -1);

    if (notification != null && !notification.isExpired()) {
      String notificationText = notification.getMessage();
      Gui.drawRect(
          mc.currentScreen.width / 2 - mc.fontRendererObj.getStringWidth(notificationText) / 2 - 3,
          4,
          mc.currentScreen.width / 2 + mc.fontRendererObj.getStringWidth(notificationText) / 2 + 3,
          4 + 3 + mc.fontRendererObj.FONT_HEIGHT + 2,
          0x64000000);
      mc.currentScreen.drawCenteredString(
          mc.fontRendererObj, notification.getMessage(), mc.currentScreen.width / 2, 4 + 3, -1);
    }
  }
        ...
        case 0:
          { // Login
            if (task == null || task.isDone()) {
              ...
              String username =
                  StringUtils.isBlank(account.getUsername()) ? "???" : account.getUsername();
              ...
              task =
                  MicrosoftAuth.login(account.getAccessToken(), executor)
                      .handle(
                          (session, error) -> {
                            if (session != null) {
                          ...
                          completed -> {
                            ...
                            notification =
                                new Notification(
                                    TextFormatting.translate(
                                        String.format(
                                            "&7Refreshing Microsoft access tokens... (%s)&r",
                                            username)),
                                    -1L);
                            return MicrosoftAuth.refreshMSAccessTokens(
                                account.getRefreshToken(), executor);
                          })
                      .thenComposeAsync(
                          msAccessTokens -> {
                            notification =
                          ...
                          xboxXstsData -> {
                            notification =
                                new Notification(
                                    TextFormatting.translate(
                                        String.format(
                                            "&7Acquiring Minecraft access token... (%s)&r",
                                            username)),
                                    -1L);
                          ...
                          mcToken -> {
                            notification =
                                new Notification(
                                    TextFormatting.translate(
                                        String.format(
                                            "&7Fetching your Minecraft profile... (%s)&r",
                                            username)),
                                    -1L);
  ...
  class GuiAccountList extends GuiSlot {
    public GuiAccountList(Minecraft mc) {
      super(
          mc,
          GuiAccountManager.this.width,
          GuiAccountManager.this.height,
          32,
          GuiAccountManager.this.height - 64,
          16);
    }
    ...
    protected void drawSlot(int entryID, int x, int y, int k, int mouseXIn, int mouseYIn) {
      ...
      GuiAccountManager.this.drawString(fr, username, x + 2, y + 2, -1);

      String tag =
          TextFormatting.translate(account.isCracked() ? "&8[&cCracked&8]" : "&8[&bPre&8]");
